...
func init() {
	// concatenation
	RegisterOperator("+", &types.SimpleFunc{
	...
	RegisterOperator("π", &types.SimpleFunc{
...
var OperatorFuncs = make(map[string][]*types.SimpleFunc) // must initialize
...
func RegisterOperator(operator string, fn *types.SimpleFunc) {
	if _, exists := OperatorFuncs[operator]; !exists {
		OperatorFuncs[operator] = []*types.SimpleFunc{} // init
	}
...
func (obj *OperatorPolyFunc) findFunc(operator string) *types.SimpleFunc {
	fns, exists := OperatorFuncs[operator]
	if !exists {
		return nil
	}
